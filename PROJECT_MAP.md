...
| UI Polish: Calendar Grid | Done | softer grid borders (opacity 40→20), increased internal cell padding (p-1.5→p-2), consistent spacing |
| UI Polish: Day Names Header | Done | font-semibold (600), darker text (zinc-600), more vertical space (py-3) |
| UI Polish: Month Title | Done | text-xl + font-extrabold + tracking-tight, main visual element in header |
| UI Polish: Today Indicator | Done | larger circle (h-8 w-8), hover:bg-emerald-600 + hover:shadow-md, primary color bg with white text |
| UI Polish: "Quiet Day" Card | Done | custom card with emerald icon circle, clear title + description + professional CTA button, rounded-xl + shadow-sm + border |
| UI Polish: Mini Calendar | Done | gap 0.5→1, py-1→py-1.5, hover:text-zinc-700 on day cells, hover:bg-emerald-600 on today, font-semibold day names, cursor-pointer + duration-150 on nav buttons |
| UI Polish: Buttons | Done | all buttons unified: cursor-pointer, duration-150, active:scale-95, focus-visible:ring-2 ring-emerald-500, consistent rounded-lg/rounded-full |
| UI Polish: Spacing | Done | right panel space-y-5→space-y-4, p-4→p-3, reduced excess whitespace while maintaining breathing room |
| UI Polish: Micro-interactions | Done | all transition-all→duration-150 across entire file, active:scale-95 on all buttons, hover:shadow-md consistently, cursor-pointer on all clickable elements, focus-visible rings for accessibility |
| Right Panel Layout Enhancement | Done | added "الأحداث القادمة" section (up to 4 upcoming tasks within 7 days, with category dots + day labels, empty state when none) + "ملخص الشهر" stats row (total/completed/overdue counts for current month) in CalendarRightPanel default view to eliminate white space below Mini Calendar, no logic changes |
| Right Panel Flexbox Full-Height Fix | Done | outer wrapper `h-full` to fill parent flex height, inner container changed from `space-y-4 p-3` to `flex h-full flex-col justify-between gap-6 p-5` (padding 12→20px, gap 16→24px), month summary stats converted from `grid grid-cols-3` to `flex flex-col` full-width cards with increased padding (p-2.5→p-3.5), all sections distributed evenly via `justify-between` to eliminate bottom whitespace |
| Full-height Flex Chain Fix | Done | fixed 3 missing levels: (1) calendar/page.tsx `motion.div` added `h-full` for explicit height; (2) right panel `motion.div` added `flex flex-col` to become a flex parent; (3) inner container changed from `h-full` to `flex-1` for proper flex child growth. Chain now: `html.h-full` → `body.h-full` → `div.flex.h-full` → ... → `motion.div.h-full.flex.flex-col` → `div.flex-1.flex-col.justify-between` |
| Fix: `flex` × `md:block` CSS conflict | Done | `flex` (display:flex) and `md:block` (display:block at md+) conflicted — responsive utility overrides base utility in Tailwind v4, causing motion.div to be `display:block` (not flex) on desktop, breaking `flex-1` on child. Fixed by changing to `md:flex md:flex-col` and removing base `flex flex-col`. |
| Calendar UX/UI Phase 1: Cell Enhancement | Done | MonthView cell completely overhauled: (1) day number + "اليوم" badge + task count in header; (2) hover effects: `md:hover:shadow-sm` on cell, `hover:shadow-md` on task cards, `group-hover:scale-110` on + button; (3) today highlighted with `ring-1 ring-emerald-200/60` + `shadow-emerald-200/50` glow; (4) + button hidden by default (`opacity-0`) appears on hover with `group-hover:opacity-100 group-hover:scale-110`; (5) tasks limited to 2 (was 3), "+N أكثر" adjusted; (6) habit indicators as colored dots; (7) progress bar at bottom of cell (`bg-emerald-400` width=%); (8) empty state "لا توجد مهام" replacing empty div; (9) MonthView wrapped with `React.memo`; (10) view transitions wrapped with `AnimatePresence mode="wait"` + fade+slide animation |
| Calendar UX/UI Phase 2: Drag & Drop | Done | Native HTML5 Drag and Drop API (zero new libraries). Task cards in MonthView made `draggable` with `onDragStart` (stores task ID in dataTransfer). Day cells accept drops via `onDragOver`/`onDrop` handlers that call `useTaskStore.updateTask(id, { dueDate })`. Visual feedback: cell shows `ring-2 ring-emerald-300 bg-emerald-50/40` during drag-over. `onDragLeave` uses `e.currentTarget.contains()` to prevent flickering with child elements. `dragOverDate` state tracked in MonthView. |
| Calendar UX/UI Phases 3+4: Heat Map + Stats + Timeline | Done | (1) **Heat Map**: thin `h-0.5` emerald bar at bottom of each MonthView cell, width proportional to task count (capped at 6 tasks = 100%). (2) **Monthly Stats Bar**: compact stat row between header and calendar grid showing total/completed/overdue/completion% for current month. (3) **Unified Timeline in DayView**: habit strip overlay at top of day timeline showing today's habits as rounded pills with icons + colors. No new libraries, TypeScript clean. |
