...
export function FocusMode() {
  const { toggleFocusMode, activeTaskId, setActiveTaskId } = useUIStore();
  const {
    tasks,
    toggleComplete,
    toggleImportant,
  ...
  const currentIdx = filtered.findIndex((t) => t.id === activeTaskId);
  const task =
    currentIdx >= 0 ? filtered[currentIdx] : filtered[0] ?? null;
  ...
  useEffect(() => {
    if (secondsLeft === 0 && !running) {
      if (phase === 'work') switchPhase('break');
      else switchPhase('work');
    }
  }, [secondsLeft, running, phase, switchPhase]);
  ...
  const goNext = () => {
    if (!task || filtered.length === 0) return;
    const i = filtered.findIndex((t) => t.id === task.id);
    ...
    >
      ...
      <div className="relative mb-8">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="font-mono text-4xl font-bold tracking-wider text-white tabular-nums md:text-5xl">
            {mm}:{ss}
          </span>
      ...
      <AnimatePresence mode="wait">
        {task && (
          ...
          >
            ...
            <button
              onClick={() => task && toggleComplete(task.id)}
              className={cn(
